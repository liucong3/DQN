...
class FastNet(Net):
	...
	def buildLoss(self):
		...
		if clipDelta:
			...
			loss = tf.reduce_sum(tf.square(deltasCliped) / 2 + self.penaltyForBounds / 2 * (tf.square(deltasL) + tf.square(deltasU)) + (tf.abs(self.deltas) + deltasL + deltasU - 3 * tf.abs(deltasCliped)) * clipDelta)
		else:
